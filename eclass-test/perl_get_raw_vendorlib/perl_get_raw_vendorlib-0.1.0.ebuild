...
DESCRIPTION="Test for perl_get_raw_vendorlib"
...
do_test() {
	...
	rawpath="$(perl_get_raw_vendorlib)"
	is_ok $? "Calling perl_get_raw_vendorlib succeeded"

	[[ -n "${rawpath}" ]]
	is_ok $? "rawpath is non-zero length (${rawpath})"

	[[ -d "${rawpath}" ]]
	is_ok $? "rawpath is a real directory";
	...
	if is_prefixed; then
		...
		printf "\0%s" "${rawpath}" | grep -aqFf <( printf "\0%s" "${EPREFIX}" )
		is_ok $? "raw vendor lib includes EPREFIX"
	fi
